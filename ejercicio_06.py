...
def imprime_nombre_y_altura_heroe():
    for superheroe in lista_personajes:
        superheroe["altura"] = float(superheroe["altura"])
        print("el nombre del superheroe es {0} y su altura es {1:.2f}".format(superheroe["nombre"],superheroe["altura"]))

def determina_nombre_superheroe_mas_alto():
    maximo_altura = float(lista_personajes[0]["altura"])
    nombre_alto = lista_personajes[0]["nombre"]
    for superheroe in lista_personajes:
        #superheroe["altura"] = float(superheroe["altura"])
        if(float(superheroe["altura"]) > maximo_altura):
            maximo_altura = superheroe["altura"]
            nombre_alto = superheroe["nombre"]

    print("El nombre del superheroe mas alto es {1} y su altura es {0} mts".format(maximo_altura, nombre_alto))

def determina_nombre_superheroe_mas_bajo():
    minimo_altura = float(lista_personajes[0]["altura"])
    nombre_bajo = lista_personajes[0]["nombre"]
    for superheroe in lista_personajes:
        if(float(superheroe["altura"]) < minimo_altura):
            minimo_altura = superheroe["altura"]
            nombre_bajo = superheroe["nombre"]

    print("El nombre del superheroe mas bajo es {1} y su altura  es {0:.2f} mts".format(minimo_altura, nombre_bajo))
...
def determina_peso_y_nombre_heroe_mas_pesado():
    ...
    for superheroe in lista_personajes:
        ...
        if(superheroe["peso"] > maximo_peso):
            maximo_peso = superheroe["peso"]
            nombre_mas_pesado = superheroe["nombre"]
    print("El nombre del superheoroe mas pesado es {0} y su peso es {1}".format(nombre_mas_pesado, maximo_peso))

def determina_peso_y_nombre_heroe_menos_pesado():
    nombre_menos_pesado = lista_personajes[0]["nombre"]
    minimo_peso = float(lista_personajes[0]["peso"])
    for superheroe in lista_personajes:
        superheroe["peso"] = float(superheroe["peso"])
        if(superheroe["peso"] < minimo_peso):
            minimo_peso = superheroe["peso"]
            nombre_menos_pesado = superheroe["nombre"]
    print("El nombre del superheoroe menos pesado es {0} y su peso es {1}".format(nombre_menos_pesado, minimo_peso))
